...
description: 
---
Last updated 08/1/24 by M.Kachmar
  - combine the two code files we have 
  -This code needs some work to make sure that all of it is working. 
  -The fence creek data file is only saying that june has 150 individuals. Is this correct? Was trying to go into the google drive to make sure everything is there but I am confused about the data sheets in there and what is what. This is an example of why it is important to keep correct data files and organization. 
...
#This code chunk is not working. It is giving an incorrect format for the dates
```{r}
#The dates in the data files are reading as a character "chr" so we need to change them to date format

#data_all <- data_all %>% mutate(Date = as.Date(Date)) 

#Create a month column in the data
#data_all <- data_all %>%
...
st_height <- summarySE(data_all%>% filter(!is.na(Shell_Height)), measurevar="Shell_Height", groupvars=c("Site", "Date"))
