...
pdf(paste0(path = "Lab_Data_TissueProcessing/output" ,"/BCS_proportion_all_2023.pdf"), height = 7, width = 13)
...
pdf(paste0(path = "Lab_Data_TissueProcessing/output" ,"/BCS_proportion_all_2024.pdf"), height = 7, width = 13)
...
BCS_proportion_GOLD <- ggplot(data=GOLD_BCS_proportions, aes(x=month, y= Proportion, fill=condition_score_bin)) +
  geom_bar(width = .5, stat="identity", position = "fill")+  
   theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+
  labs(title="Gold Star Beach Proportion of body condition scores - 2023", x ="month", y = "Proportion of body condition scores ")+ theme(axis.title.y = element_text(size = rel(1.3), angle =90), axis.title.x = element_text(size = rel(1.3), angle = 0))+
  theme(axis.text=element_text(size=12))+
 #scale_fill_brewer() +
  facet_wrap(~ year)
BCS_proportion_GOLD
