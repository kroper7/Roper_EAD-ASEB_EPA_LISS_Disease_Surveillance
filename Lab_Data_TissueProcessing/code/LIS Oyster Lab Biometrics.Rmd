...
Updated 04/15/24 by M. Kachmar
  - QAQC 0424ASHC - height slightlight larger than March - weather conditions were difficult so we were just grabbing what we could - still under 120mm
  -BCS seem to be slightly improving this month - more 3 scores. 
...
BCS_proportion_all<- ggplot(data=df_BCS_proportions, aes(x=month, y= Proportion, fill=condition_score_bin)) +
  ...
  labs(title="Proportion of body condition scores April- November 2023", x ="month", y = "Proportion of body condition scores ")+ theme(axis.title.y = element_text(size = rel(1.3), angle =90), axis.title.x = element_text(size = rel(1.3), angle = 0))+
...
GOLD_BCS_percentage <- GOLD_precent_greater_3 
  ...
  #ylim(0,60)+ 
  scale_y_continuous(limits = c(0,100), breaks = seq(0,100, by = 10)) + facet_wrap(~year)
...
ggplot(data=df_GOLD, aes(x= condition)) +
  ...
  labs(title="Goldstar Beach Body Condition Score ", x ="Condition categorization")+
    facet_wrap(~ month + year, scales = "free")
...
BCS_proportion_all<- ggplot(data=df_BCS_proportions, aes(x=month, y= Proportion, fill=condition_score_bin)) +
  ...
  labs(title="Proportion of body condition scores May - November 2023", x ="Month", y = "Proportion of body condition scores ")+ theme(axis.title.y = element_text(size = rel(1.3), angle =90), axis.title.x = element_text(size = rel(1.3), angle = 0))+
  theme(axis.text=element_text(size=12))+
  labs(fill ="Body Condition Score")+
  facet_wrap(~ site )
BCS_proportion_all+scale_fill_manual(values = c("#003C30","#01665E","#80CDC1","#C7EAE5","azure1","slategray1","slategray3", "slategray4", "gray25" ))
