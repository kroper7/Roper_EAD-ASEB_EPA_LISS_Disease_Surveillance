...
Last updated 4/17/24 by M. Kachmar
  2024 data can merge to itself but it is still a character. When I try to convert it to a date in a specific format it either changes the year to 2019 - why? idk? or causes NA. SOS. 
...
for (m in 1:nrow(file.names.table)) {
  ...
  Master_Table1 <- rbind(raw_df,Master_Table1) #bind to a cumulative list dataframe
}
 View(Master_Table)
...
file.names.2024<- data.frame(txt.files = 
                                         (basename(list.files(path = "raw_data/2024", 
                                                              pattern = "csv$", 
                                                              recursive = FALSE))))

Master_Table2 <- data.frame()
...
Master_Table2 <- rbind(raw_df_2024, Master_Table2)
...
x <- mdy_hm(raw2$`Date Time`)



######### Merging 2023 and 2024 for a master sheet
Master_Table <- rbind(Master_Table1, Master_Table2)
View(Master_Table)
