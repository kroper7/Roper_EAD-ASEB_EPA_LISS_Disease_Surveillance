...
Last updated 7/15/2024 by M.Kachmar
-I accidentally uploaded the wrong sheet and the ash creek sonde is NOT skipping data. 
...
cor(CT_Temperature_2023, method = c("pearson"))
```
...
```{r}
#Separating Sonde data by year at ASHC
CT_Temperature <- CT_Temperature %>% dplyr::mutate(`Date Time`= as.Date(`Date Time`), Year = year(`Date Time`))
...
CT_Temperature_ASHC. <- CT_Temperature_ASHC %>% na.omit() %>%
             ggplot(aes(x=`Date Time`, 
                       y=mean, # the mean values
                       group=Site,
