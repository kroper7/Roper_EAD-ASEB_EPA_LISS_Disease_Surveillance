...
######unable to get code for this piece to work####
#Error: Warning: Detected an unexpected many-to-many relationship between `x` and `y`.Warning: Detected an unexpected many-to-many relationship between `x` and `y`.Warning: Detected an unexpected many-to-many relationship between `x` and `y`.#####

FENCdensitysummary_zone <- subset(fenc_density_sum_all_zone, select = -c(sd,se,ci)) %>%
    left_join(subset(fenc_density_sum_live_zone, select = -c(N,sd,se,ci,Site_Name)), by = "Bed_Section", "Quadrat_Zone") %>%
    left_join(subset(fenc_density_sum_box_zone, select = -c(N,sd,se,ci,Site_Name)), by = "Bed_Section", "Quadrat_Zone") %>%
    left_join(subset(fenc_density_sum_gaper_zone, select = -c(N,sd,se,ci,Site_Name)), by =  "Bed_Section", "Quadrat_Zone") 



FENCdensitysummary2 <- rbind(ASHCdensitysummary,ASHCdensitysummary_zone)
