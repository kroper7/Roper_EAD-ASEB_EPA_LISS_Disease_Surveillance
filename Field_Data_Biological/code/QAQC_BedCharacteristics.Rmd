...
Last updated: 6/11/2024 by M. Kachmar
- updated ASH height measurements for spring 2024
...
#Bed Area
```{r}
#Bed area data CSV file

bed_area <- read_csv("Field_Data_Biological\\raw_data\\Bed_area\\BedArea - Master.csv",col_names = TRUE)

bed_area <- bed_area%>% filter(!is.na(Site))

bed_area$Date_collected<- mdy(bed_area$Date_collected)

bed_area$year<- year(bed_area$Date_collected)
head(bed_area)

bed_area

#Loading packages to read and view .KML files
...
#Read .KML file
ASHC_AreaRight_2024<- st_read("Field_Data_Biological\\raw_data\\Bed_area\\0624ASHC_Right.kml")
  
ASHC_AreaRight_2024 %>%
  glimpse()

attributes <- lapply(X = 1:nrow(ASHC_AreaRight_2024), 
                     FUN = function(x) {
