...
#pulling the temp data from sonde from one day from all three sites
...
#can you just add "addheatmaps" to this and then it would work? -> have to delete N/A from coordinates
leaflet(tempmap) %>%
  # Add first tile
  addTiles(group="One") %>%
  # Add second tile
  addProviderTiles(providers$Esri.WorldImagery, group="Two") %>% 
  #add heat map 
  addHeatmap(lng = tempmap$Long, lat = tempmap$Lat, blur = 40, max = 0.05, radius = 15, group = "Site") %>%
  # Add Layer controls
  addLayersControl(baseGroups=c("One", "Two"), 
                   overlayGroups=c("Fence Creek", "Ash Creek", "Goldstar"), 
                   options=layersControlOptions(collapsed=FALSE))
...
#not sure if we need to remove the na in the lat and long
heatedmap <- tempmap %>% filter(!is.na(tempmap$Lat))

tempmap1<- ggplot(tempmap,aes(x = Long, y= Lat, group=Site))+
...
ggplot(
  tempmap = tempmap,
  mapping = aes(x = tempmap$Date, y = tempmap$Temperature)
) +
  geom_point(aes(color = tempmap$Site, shape= tempmap$Site)) +
