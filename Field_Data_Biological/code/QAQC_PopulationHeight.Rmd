...
Last updated 9/25/24 M. Kachmar
...
FENC_Distribution_Dead<-ggplot(data=height_FENC_Dead, aes(x= Height)) +
  ...
  theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  labs(title="Fence Creek Height Dead Oysters ", x ="Height (mm)")+ scale_x_continuous(limits= c(0,190), breaks = seq(0,190, by =10) )+
...
ggplot(data=df_quadrat_GOLD, aes(x= Survival_status, y = Count, fill = Survival_status)) +
  ...
  facet_wrap(~ Season+Year, scales = "free")
```
