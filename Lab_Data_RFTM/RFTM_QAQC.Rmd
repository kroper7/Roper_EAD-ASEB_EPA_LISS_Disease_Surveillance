...
df_intensity_proportions<- data_all %>%
  mutate(rftm_final_numeric = as.numeric(rftm_final),intensity_bin = cut(rftm_final, breaks = c(0,0.5, 1, 2, 3, 4, 5), labels=c("0","0.5","1","2","3","4","5"))) %>%
...
ggplot(data=df_intensity_proportions, aes(x=date_collected, y= Proportion, fill=intensity_bin)) +
  geom_bar(width = 6, stat="identity", position = "fill")+  
   theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+
  labs(title="Proportion of Intensity (population) scores of Dermo infection", x ="Date", y = "Proportion")+ theme(axis.title.y = element_text(size = rel(1.3), angle =90), axis.title.x = element_text(size = rel(1.3), angle = 0))+
  theme(axis.text=element_text(size=12))+
  facet_wrap(~ site)
...
ggplot(data=df_ASHC, aes(x=date_collected, y=rftm_final, group= date_collected)) +
  geom_boxplot()+  
   theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+
  labs(title="Ash Creek Weighted Prevalence Dermo infection", x ="Date", y = "Weighted Prevalence")+ theme(axis.title.y = element_text(size = rel(1.3), angle =90), axis.title.x = element_text(size = rel(1.3), angle = 0))+
  theme(axis.text=element_text(size=12))
