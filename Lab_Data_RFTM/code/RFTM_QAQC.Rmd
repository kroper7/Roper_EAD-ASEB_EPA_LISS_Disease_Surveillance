...
---
Last updated 5/31/24 by M. Kachmar
  - Accuracy 78.5%
  - All data up to date. STAR vs GOLD look the same. While STAR has ~10% 0.5 it has a bigger sample set. I would not consider this significantly different. 
...
df_STAR <- df_STAR %>% filter(!is.na(rftm_final))

#Weighted prevalence
st_intensity_STAR <- summarySE(df_STAR, measurevar="rftm_final", groupvars=c("month", "year"))
st_intensity_STAR

#Prevalence
df_STAR_prevalence <- df_prevalence_summary %>% 
  filter(site == "STAR")
df_STAR_prevalence

#Intensity Index
df_STAR_intensity <- df_intensity_summary %>%
  filter(site == "STAR")
df_STAR_intensity


df_STAR_proportions<-df_intensity_proportions %>%
  filter(site =="STAR")
...
ggplot(data=df_STAR, aes(x=month, y=rftm_final, group= date_collected)) +
  geom_boxplot()+  
   theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+
  labs(title="Gold Star Beach 2023 planting Weighted prevalence of Dermo infection", x ="Date", y = "Weighted Prevalence")+ theme(axis.title.y = element_text(size = rel(1.3), angle =90), axis.title.x = element_text(size = rel(1.3), angle = 0))+
  theme(axis.text=element_text(size=12))+ facet_wrap(~ year, scales = "free", ncol=1)

#Weighted prevalence
ggplot(data=st_intensity_STAR, aes(x=month, y=rftm_final)) +
  geom_point()+  
  geom_line()+
   theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+
...
##Prevalence
ggplot(data=df_STAR_prevalence, aes(x=month, y=prevalence_precentage)) +
  geom_point()+  
  geom_line()+
   theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+
...
#Intensity index
ggplot(data=df_STAR_intensity, aes(x=month, y=intensity)) +
  geom_point()+  
  geom_line()+
   theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+
...
ggplot(data=df_STAR_proportions, aes(x=month, y= Proportion, fill=intensity_bin)) +
  geom_bar(width = .5, stat="identity", position = "fill", colour = "black")+  
   theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+
  labs(title="Proportion of Intensity scores of Dermo infection at Gold Star 2023 planting", x ="month", y = "Proportion", fill = "Intensity Score")+ theme(axis.title.y = element_text(size = rel(1.3), angle =90), axis.title.x = element_text(size = rel(1.3), angle = 0))+
  theme(axis.text=element_text(size=12))+
   scale_fill_brewer() +
   scale_x_continuous( breaks = seq(4,12, by =1) )+
  facet_wrap(~year)
