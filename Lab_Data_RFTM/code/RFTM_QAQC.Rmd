...
Last updated 3/25/24 by M. Kachmar
  - CT march uploaded & QAQC complete
  - Accuracy %76.6
  - edited graphing/code to reflect year as we are in year 2 (exciting!)
...
knitr::opts_knit$set(root.dir = 'C/Users/kelly.roper/Documents/Github/EAD-ASEB_LISS_Disease_Surveillance')    #kelly's
```
...
data_cond <- list.files(path = "Lab_Data_TissueProcessing/raw_data/Files_by_Month",                           # Identify all CSV files
  pattern = "*.csv", full.names = TRUE) %>% 
  lapply(read_csv) %>%     # Store all files in list
  bind_rows               # Combine data sets into one data set 
data_all   
...
data_cond <- select(data_cond, -dissection_notes, -light_regime, -oyster_zone, -...21:-...33)
...
data_rftm.cond_prop$date_collected <- factor(data_rftm.cond_prop$month, levels = c("4","5","6","7", "8", "9", "10", "11"),
