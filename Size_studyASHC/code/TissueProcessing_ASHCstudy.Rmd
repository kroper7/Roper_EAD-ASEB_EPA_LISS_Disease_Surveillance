...
ggplot(data=subset(all_data_prop_size, condition_score < 4), aes(x=condition_score, y=Proportion, fill=rftm_score))+
  ...
  theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+
  labs(title="Proportion of RFTM scores vs. Body Condition Scores by Date Collected & Size Class", x ="condition score", y = "Proportion")+ theme(axis.title.y = element_text(size = rel(1.3), angle =90), axis.title.x = element_text(size = rel(1.3), angle = 0))+
  theme(axis.text=element_text(size=12))+
 scale_fill_brewer("RFTM Score")+
  facet_wrap(~sizeclass)+
   scale_x_continuous("Body Condition Score", breaks = c(1,2,3,4,5,6,7,8,9))+
...
all_data_dates_prop$date_collected <- factor(all_data_dates_prop$date_collected, levels = c("2023-04-23","2023-05-10","2023-06-12","2023-07-10", "2023-08-07", "2023-08-23", "2023-09-05", "2023-10-03", "2023-11-06"),
...
ggplot(all_data_dates_prop, aes(x=condition_score, y=Proportion, fill=rftm_score))+
  ...
  labs(title="Proportion of Intensity scores within Body Condition Scores by Date Collected & Size Class", x ="condition score", y = "Proportion")+ theme(axis.title.y = element_text(size = rel(1.3), angle =90), axis.title.x = element_text(size = rel(1), angle = 0))+
...
ggplot(data=subset(all_data_dates_prop, condition_score < 4), aes(x=condition_score, y=Count, fill=rftm_score))+
  geom_bar(width = .8, stat="identity", position = "fill",colour = "black")+
  theme(panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank())+ 
...
```{r}
#Facet wrap for body condition >3 vs. proportion seperated by size class & date collected
ggplot(data=subset(all_data_dates_prop, condition_score < 4), aes(x=condition_score, y=Count, fill=rftm_score))+
  geom_bar(width = .8, stat="identity", position = "fill",colour = "black")+
 scale_fill_brewer(palette = "YlOrBr")+
  facet_wrap(~sizeclass)+
