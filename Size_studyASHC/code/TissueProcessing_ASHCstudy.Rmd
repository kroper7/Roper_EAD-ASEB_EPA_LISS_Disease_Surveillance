...
p$date_collected <- as.Date(p$date_collected)
...
mean_rftm_score <- all_data_dates %>%
  dplyr::group_by(date_collected)%>%
  dplyr::summarize(mean_bsc = mean(rftm_final, na.rm = TRUE))
...
```{r}
#Line graph of mean body condition for each month ASHC
mean_rftm_score$date_collected <- factor(mean_rftm_score$date_collected, levels = c("2023-04-23","2023-05-10","2023-06-12","2023-07-10", "2023-08-07", "2023-08-23", "2023-09-05", "2023-10-03", "2023-11-06"),
        labels=c("April", "May", "June", "July", "Aug", "Size Study", "Sept", "Oct", "Nov"))

ggplot(data = mean_rftm_score, aes(x=date_collected, y=mean_bsc))+
...
mean_all$empty_column <- mean_rftm_score$mean_bsc
