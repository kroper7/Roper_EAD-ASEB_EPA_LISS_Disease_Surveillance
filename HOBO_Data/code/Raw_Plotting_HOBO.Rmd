...
df2<-read_csv("HOBO_Data\\raw_data\\092024_FENC_HOBO.csv", col_names = TRUE)%>%  # update this csv when new data comes in from the new HOBO
  dplyr::rename(Temperature = `Ch:1 - Temperature   (°C)`) %>%
  dplyr::rename(Date_Time = `Date-Time (EST/EDT)`)%>%
  dplyr::select( -c("Host Connected", "Button Down", "Button Up", "End of File", "Water Detect"))
