...
#Condition distribution by collection date
p_ASHC<-ggplot(data=df_ASHC, aes(x= condition, fill=date_collected)) 
p_ASHC +geom_density(alpha=0.5)+  #scale_fill_manual(values=c("forestgreen","orange", "purple"))+
  theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  labs(title="Body Condition index ", x ="Condition categorization")
```
