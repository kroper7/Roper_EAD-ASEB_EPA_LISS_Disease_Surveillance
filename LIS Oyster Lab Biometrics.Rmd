...
ASHC_precent_greater_3 <- df_ASHC %>%
  ...
  dplyr::summarise(Percentage = mean(condition_score > 3, na.rm = TRUE)*100)
ASHC_precent_greater_3

ASHC_BCS_precentage<- ASHC_precent_greater_3 %>% na.omit() %>%
  ggplot(aes(x=month, y= Percentage)) +
  geom_bar(width = .5, stat="identity", position = "fill")+  
   theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+
  labs(title="Ash Creek % of body condition scores >3 June- November", x ="month", y = "Precentage of body condition scores >3 ")+ theme(axis.title.y = element_text(size = rel(1.3), angle =90), axis.title.x = element_text(size = rel(1.3), angle = 0))+
  theme(axis.text=element_text(size=12))
