...
ggplot(data=height_ASHC, aes(x= Height)) +
...
#Quadrat size is 25 x25cm = .25x.25m = 0.0625 m^2
Q_area_ASHC <- 0.0625

df_density_ASHC <- df_quadrat_ASHC %>%
  dplyr::group_by(Site, Season, Quadrat, Survival_status) %>% 
  dplyr::summarize(Density = sum(Count)/Q_area_ASHC)
df_density_ASHC
...
ggplot(data=df_density_ASHC, aes(x= Survival_status, y = Density, fill = Survival_status)) +
  geom_bar( stat="identity")+  #scale_fill_manual(values=c("forestgreen","orange", "purple"))+
  theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
...
df_density_FENC <- df_quadrat_FENC %>%
  dplyr::group_by(Site, Season, Quadrat, Survival_status) %>% 
  dplyr::summarize(Density = sum(Count)/Q_area_FENC)
df_density_FENC
...
#Density of oysters per meter squared
ggplot(data=df_density_FENC, aes(x= Survival_status, y = Density, fill = Survival_status)) +
  geom_bar( stat="identity")+  #scale_fill_manual(values=c("forestgreen","orange", "purple"))+
  theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
